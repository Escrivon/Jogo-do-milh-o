...
void carregarPerguntasCSV(char* nomeArquivo) {
    ...
    while (!feof(arq)) {
        ...
        if (fscanf(arq, " %199[^,],%99[^,],%99[^,],%99[^,],%99[^,], %c,%d\n",
            ...
            p.ativo = 1;
            perguntas = realloc(perguntas, (p.totalPerguntas + 1) * sizeof(Pergunta));
            perguntas[p.totalPerguntas++] = p;
...
void salvarBinario(char* nomeArquivo) {
    FILE* arq = fopen(nomeArquivo, "wb");
    if (!arq) return;
    fwrite(&p.totalPerguntas, sizeof(int), 1, arq);
    fwrite(perguntas, sizeof(Pergunta), p.totalPerguntas, arq);
    fclose(arq);
}//salva em binário;

void carregarBinario(char* nomeArquivo) {
    FILE* arq = fopen(nomeArquivo, "rb");
    if (!arq) return;
    fread(&p.totalPerguntas, sizeof(int), 1, arq);
    perguntas = malloc(p.totalPerguntas * sizeof(Pergunta));
    fread(perguntas, sizeof(Pergunta), p.totalPerguntas, arq);
    fclose(arq);
}//carrega o arquivo em binário;

void exportarCSV(char* nomeArquivo) {
    FILE* arq = fopen(nomeArquivo, "w");
    if (!arq) return;
    for (int i = 0; i < p.totalPerguntas; i++) {
...
void adicionarPergunta() {
    ...
    perguntas = realloc(perguntas, (p.totalPerguntas + 1) * sizeof(Pergunta));
    perguntas[p.totalPerguntas++] = p;//add a pergunta;
...
void excluirPergunta(int indice) {
    if (indice < 0 || indice >= p.totalPerguntas) {
        printf("Índice inválido!\n");
        return;
    }//recebe o indice da pergunta, no caso o valor do vetor, se for indice 0, irá remover a pergunta 1, etc...;

    for (int i = indice; i < p.totalPerguntas - 1; i++) {
        perguntas[i] = perguntas[i + 1];
    }
    p.totalPerguntas--;
    perguntas = realloc(perguntas, p.totalPerguntas * sizeof(Pergunta));//ajusta o tamanho da struct
